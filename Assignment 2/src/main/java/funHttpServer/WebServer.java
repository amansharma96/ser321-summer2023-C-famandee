    ...
    try {
      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
          ...
          // Logins
          for (int i = 0; i < newObject.length(); i++) {
            JSONObject ownerObject = newObject.getJSONObject(i).getJSONObject("owner");
            builder.append("Login: ");
            builder.append(ownerObject.getString("login"));
          }
        ...
        // Request 1: Round Integers
          ...
          if (parts.length != 3) {
            ...
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Invalid request format: must use round/{num1}/{num2}");
          } else {
            try {
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Result: num1 * num2 = " + result);
            } catch (Exception e) {
              // Error handling for invalid values
              builder.append("HTTP/1.1 406 Not Acceptable\n");
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Error: Must provide valid floating-point values");
            }
          }
        }

        // Request 2: Glue Integers
        else if (request.contains("glue/")) {
          String[] parts = request.split("/");
          if (parts.length != 3) {
            // Invalid request format
            builder.append("HTTP/1.1 400 Bad Request\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Invalid request format: must use glue/{num1}/{num2}");
          } else {
            try {
              Integer num1 = Integer.parseInt(parts[1]);
              Integer num2 = Integer.parseInt(parts[2]);
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Result: num1 + num2 (glued) = " + result);
            } catch (NumberFormatException e) {
              // Error handling for invalid numeric values
              builder.append("HTTP/1.1 400 Bad Request\n");
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Invalid numeric values: num1 and num2 must be valid integers");
            } catch (Exception e) {
              // Error handling for other exceptions
              builder.append("HTTP/1.1 500 Internal Server Error\n");
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("An error occurred while processing the request");
            }
          }
        }
